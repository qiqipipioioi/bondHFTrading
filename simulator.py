...
from utils import Order, CancelOrder, AnonTrade, OwnTrade, OrderbookSnapshotUpdate, FutureSnapshotUpdate, MarketOrder, \
...
class Sim:
    ...
    def update_last_trade(self) -> None:
        ...
        if not self.md.trade is None:
            self.trade_price = self.md.trade.price
    ...
    def update_action(self, action:Union[Order, CancelOrder]) -> None:
        ...
        if isinstance(action, Order):
            ...
            if action.id_to_delete in self.wait_close_orders:
                self.wait_close_orders.pop(action.id_to_delete)
    ...
    def execute_last_order(self) -> None:
        ...
        if not executed_price is None:
            ...
            if self.last_order.order_id in self.wait_close_orders:
                self.wait_close_orders[self.last_order.order_id][1] = 1
        else:
    ...
    def execute_orders(self) -> None:
        ...
        for order_id, order in self.ready_to_execute_orders.items():
            ...
            elif order.side == 'BID' and order.price <= self.trade_price:
                executed_price = order.price
                execute = 'TRADE'    
            #
            elif order.side == 'ASK' and order.price >= self.trade_price:
                executed_price = order.price
                execute = 'TRADE'
            ...
            if not executed_price is None:
                ...
                #add order to strategy update queue
                self.strategy_updates_queue.push(executed_order.receive_ts, executed_order)
                if order_id in self.wait_close_orders:
                    self.wait_close_orders[order_id][1] = 1
    ...
    def place_order(self, ts:float, size:float, side:str, price:float, order_type: str, compaire_id = -1) -> Order:
        #добавляем заявку в список всех заявок
        order = Order(ts, ts + self.latency, self.get_order_id(), side, size, price, order_type, compaire_id)
