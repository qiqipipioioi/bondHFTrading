...
from diStrategy import BestPosStrategy
...
strategy = BestPosStrategy(delay, hold_time, min_pos=min_pos, window=window)
...
plt.title("国债期货领先策略 vol=3 PnL", fontsize=15)
plt.savefig('国债期货领先策略 vol=3 PnL.png', dpi=300)
#plt.show()
'''
...
plt.ylabel("PnL", fontsize=13)
plt.title("国债期货领先策略  vol=3 日内 PnL", fontsize=15)
plt.savefig('国债期货领先策略  vol=3 日内 PnL.png', dpi=300)
#plt.show()
'''
