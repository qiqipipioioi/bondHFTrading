...
@dataclass
class FutureSnapshotUpdate:  # future Orderbook tick snapshot
    exchange_ts : float
    receive_ts : float
    ask1: float  # tuple[price, size]
    bid1: float
    volume: float  
...
class MdUpdate:  # Data of a tick
    ...
    future: Optional[FutureSnapshotUpdate] = None
...
def update_best_positions(best_bid:float, best_ask:float, md:MdUpdate) -> Tuple[float, float]:
    if not md.orderbook is None:
        #判断是否为nan
        if (not math.isnan(md.orderbook.bids[0][0])) and (not math.isnan(md.orderbook.asks[0][0])):
            if md.orderbook.bids[0][0] >= md.orderbook.asks[0][0]:
